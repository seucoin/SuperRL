...
class Model_Run(object):
    ...
    def __init__(self, arg):
        ...
        self.form = args.form
        if args.form == "Pre-Train":
            prefix = ".json"
            args.random_embed = False
        else:
            prefix = "_in_train.json"
            args.random_embed = True
        
        self.graph_path = args.datapath + '/path_graph.json'
        self.train_path = args.datapath + '/train_tasks' + prefix
        ...
        self.e1rel_e2 = json.load(open(args.datapath + '/e1rel_e2'+prefix))
        ...
        self.rel2candidates = json.load(open(args.datapath + '/rel2candidates'+prefix))
    ...
    def train(self, args):
        ...
        for data in train_generate(self.form, args.datapath, args.batch_size, args.few, self.symbol2id, self.ent2id, self.e1rel_e2):
            ...
            support_meta = self.get_meta(support_left, support_right) # ((5,100,2),(5),(5,100,2),(5))
            query_meta = self.get_meta(query_left, query_right)# ((512,100,2),(512),(512,100,2),(512))
            false_meta = self.get_meta(false_left, false_right)# ((512,100,2),(512),(512,100,2),(512))
